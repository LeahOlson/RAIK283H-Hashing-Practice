...
let hashMultiplicativeMethod = (key, array_size) => {
    return (key * 41) % array_size;
}

let hashAdditiveMethod = (key, array_size) => {
    let folds = getFolds(key, 2);
    let sum = 0;
    folds.forEach(x => {
        sum += x;
    })
...
let hashPowerMethod = (key, array_size) => {
    return Math.pow(key,2) % array_size;
}
...
$(document).ready(function () {
    ...
    results.push(runExperiment(hashMultiplicativeMethod, "multiplicative modulus", collisionLinearProbing, "linear probing", .5));
    results.push(runExperiment(hashAdditiveMethod, "additive modulus", collisionLinearProbing, "linear probing", .5));
    results.push(runExperiment(hashPowerMethod, "power modulus", collisionLinearProbing, "linear probing", .5));
